...
def submain_matching_par(N,FileName,short_name,OutputPath,mz_bin_size1,rt_cut,dt_cut,listVarMatch,SelShowVar,Align_RT_name,selectIO,ionNumThresh):
    ...
    allVariableName = listVarMatch+SelShowVar
    # save variables into file
    # header1 = header.tolist()
    mz_bin_split1 = mz_bin_split.tolist()
    outputArray = {'mz_min':mz_min,'mz_max':mz_max,'dt_min':dt_min,'dt_max':dt_max,'rt_min':rt_min,'rt_max':rt_max,
                   'nfeature':nfeature,'numFile':numFile,'mz_bin_split':mz_bin_split1,'allVariableName':allVariableName,'short_name':short_name }

    with open(path_Mat+'\myVariable.json', 'w') as f:
        json.dump(outputArray, f)
